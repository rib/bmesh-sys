...
/* Forward declaration of the cubic-Bezier forward-difference sampler. The
 * standalone build provides its own scalar implementation of this symbol, so
 * the full curve module (BKE_curve.hh) is not part of this translation unit's
 * include closure. */
...
/* NOTE: BKE_curveprofile_blend_write / BKE_curveprofile_blend_read are omitted
 * from this standalone build. They are the only functions in the upstream file
 * that depend on the blend-file serialization layer (BLO_read_write), which is
 * out of scope here. */
