...
extern "C"
{
    ...
    /* Copy each face's header flags (f->head.hflag) into out[], one int per
     * face, in BM_FACES_OF_MESH iteration order (the same order bms_snapshot
     * writes face data). The flags are a bitfield (BM_ELEM_SELECT,
     * BM_ELEM_SMOOTH, etc.). Up to out_cap values are written; the true face
     * count is always returned, so callers can detect truncation when the
     * count exceeds out_cap. out may be null when out_cap is 0 to obtain just
     * the count. */
                           ...
                           bool use_smooth)
    {
        ...
        if (!BMO_op_initf(bm,
                          &op,
                          BMO_FLAG_DEFAULTS,
                          "edgeloop_fill edges=%eb mat_nr=%i use_smooth=%b",
                          reinterpret_cast<BMHeader **>(edges),
                          edges_len,
                          mat_nr,
                          use_smooth))
    ...
    bool bms_edgenet_prepare(BMesh *bm,
                             BMEdge **edges, int edges_len)
    {
        BMOperator op;
        if (!BMO_op_initf(bm,
    ...
    int bms_edgenet_prepare_out(BMesh *bm,
                                BMEdge **edges, int edges_len,
                          ...
                          int mat_nr, bool use_smooth, int sides)
    {
        ...
        if (!BMO_op_initf(bm,
                          &op,
                          BMO_FLAG_DEFAULTS,
                          "edgenet_fill edges=%eb mat_nr=%i use_smooth=%b sides=%i",
                          reinterpret_cast<BMHeader **>(edges),
                          edges_len,
                          mat_nr,
                          use_smooth,
                                 ...
                                 bool use_normals, bool use_data)
    {
        ...
        if (!BMO_op_initf(bm,
                          &op,
                          BMO_FLAG_DEFAULTS,
                          "face_attribute_fill faces=%eb use_normals=%b use_data=%b",
                          reinterpret_cast<BMHeader **>(faces),
                          faces_len,
                          use_normals,
                          use_data))
                                    ...
                                    BMFace **out_buf, int out_cap)
    {
        ...
        BMOpSlot *slot = BMO_slot_get(op.slots_out, "faces_fail.out");
        const int n = slot->len;
        const int n_copy = (n < out_cap) ? n : out_cap;
        BMFace **slot_items = reinterpret_cast<BMFace **>(slot->data.buf);
        for (int i = 0; i < n_copy; ++i)
